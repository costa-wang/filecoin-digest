...
func main() {
	var parentPath = `E:\golang_workspace\src\practice\go-filecoin`
	datastore, err := NewDatastore(parentPath)
	if err != nil {
		return
	}
	var currPath string = "go_datastore_fs"
	var key = ds.NewKey(currPath)
	var value []byte = []byte(currPath)
	datastore.Put(key, value)
	size, err := datastore.GetSize(k)
	if err != nil {
		return
	}
	fmt.Println(size)
	
	// du, err := datastore.DiskUsage()
	// if err != nil {
	// 	return
	// }
	// fmt.Println(du)
